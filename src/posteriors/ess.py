# code from https://github.com/wjmaddox/drbayes/blob/master/subspace_inference/posteriors/ess.py
...
class EllipticalSliceSampling(torch.nn.Module):
    ...
    def prior_sample(self, prior='identity', scale=1.0):
        if prior == 'identity':
            ...
            trans_cov_mat = self.subspace.cov_factor.matmul(
                self.subspace.cov_factor.subspace.t()).numpy()
            trans_cov_mat /= (self.swag_model.n_models.item() - 1)
            cov_mat = np.eye(self.subspace.cov_factor.size(0)) + trans_cov_mat

        else:
            raise NotImplementedError(
                'Only schur and identity priors have been implemented')

        cov_mat *= scale
        sample = np.random.multivariate_normal(
            np.zeros(self.subspace.cov_factor.size(0)),
            cov_mat.astype(np.float64), 1)[0, :]
        return sample
    ...
    def log_pdf(self, params, temperature=1., minibatch=False):
        ...
        with torch.no_grad():
            proj_model = ProjectedModel(model=self.base_model,
                                        subspace=self.subspace,
                                        proj_params=params_tensor)
            loss = 0
            num_datapoints = 0.0
    ...
    def fit(self, use_cuda=True, prior='identity', scale=1.0, **kwargs):
        ...
        for i in range(self.num_samples):
            ...
            current_sample, logprobs[i] = self.slice_method(
                initial_theta=current_sample,
                prior=prior_sample,
                lnpdf=self.log_pdf,
                **kwargs)
            # print(logprobs[i])
    ...
    def sample(self, ind=None, *args, **kwargs):
        ...
        self.model = ProjectedModel(model=self.base_model,
                                    subspace=self.subspace,
                                    proj_params=rsample)
        return rsample
